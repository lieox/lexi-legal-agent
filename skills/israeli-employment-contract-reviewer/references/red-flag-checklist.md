# Israeli Employment Contract Red-Flag Checklist

Complete checklist for pre-signing review. Use alongside the main SKILL.md instructions.

## Category A: Blockers (Do Not Sign Until Fixed)

These findings are illegal or grossly unfair. The contract must be amended before signing.

| # | Clause pattern | Legal basis | Why it is a blocker |
|---|---------------|-------------|---------------------|
| A1 | Pension contribution less than 6.5% benefits or less than 6% employee / 6%-8.33% severance | Mandatory Pension Expansion Order 2008 | Employer cannot contract below the statutory minimum |
| A2 | "At-will employment" without subordination to Israeli law | No such concept in Israeli labor law | Clause is unenforceable and signals bad drafting |
| A3 | Hoda'at Mukdemet below statutory minimum by tenure | Prior Notice Law 2001 | Cannot contract below the minimum |
| A4 | Salary not clearly stated as brutto (gross) or netto (net) | Written Employment Terms Notice Law 2002 | Ambiguity is itself a violation |
| A5 | Clauses waiving the employee's rights under cogent labor law | General principle | Any waiver of cogent rights is void |
| A6 | No written contract offered despite explicit request | Written Employment Terms Notice Law 2002 | Employer must provide terms in writing |
| A7 | Conditioning employment on pregnancy status, marriage status, age, or religion | Employment Equal Opportunities Law 1988 | Prohibited basis for hiring |
| A8 | Forced arbitration in a foreign jurisdiction | Labor Courts Law | Israeli labor courts have exclusive jurisdiction for many disputes |

## Category B: Major (Negotiate Before Signing)

Below market standard or legally questionable. Push back before signing.

| # | Clause pattern | Recommended negotiation |
|---|---------------|-------------------------|
| B1 | Non-compete over 12 months | Reduce to 6 months, narrow geographic scope, narrow definition of "competitor" |
| B2 | Non-compete without compensation during restriction | Remove, or require compensation = 100% of salary during restriction |
| B3 | Section 14 (Saif 14) waiver without matching salary definition | Require "salary" to include all fixed components subject to pension |
| B4 | Invention assignment covering "all inventions" | Narrow to inventions made during work hours with work resources, related to the employer's business |
| B5 | Discretionary bonus without trigger | Convert to formulaic tied to KPIs, or confirm baseline |
| B6 | Working hours over 42/week without overtime clause | Add explicit 125%/150% overtime per Hours of Work and Rest Law |
| B7 | No keren hishtalmut in a tech role | Add with 7.5% employer / 2.5% employee split |
| B8 | Probation over 12 months | Reduce to 3-6 months |
| B9 | Vacation days below statutory (12 days/yr years 1-4, rising to 23 after year 5) | Increase to at least statutory minimum |
| B10 | Sick days below 18/yr accrual | Increase to 1.5 days/month = 18/year |
| B11 | Dmei havra'a clause absent | Add per Extension Order |
| B12 | Broad confidentiality extending beyond 5 years | Scope to trade secrets only, time-limited |

## Category C: Minor (Note But Acceptable)

Not ideal but common and not legally defective.

| # | Clause pattern | Note |
|---|---------------|------|
| C1 | Class of employer vehicle benefit not specified | Ask for clarification |
| C2 | Stock options mentioned without vesting schedule | Request written vesting schedule separately |
| C3 | "Employee handbook" referenced but not attached | Request a copy |
| C4 | Relocation clause without compensation | Check if relocation is realistic |
| C5 | Dispute resolution preferring mediation before court | Acceptable if labor court remains available |

## Category D: Missing Mandatory Items (Must Be Added)

If any of these are absent, request them:

- [ ] Exact start date
- [ ] Job title and scope
- [ ] Reporting structure (direct manager)
- [ ] Work location (office, hybrid, remote)
- [ ] Gross monthly salary stated explicitly
- [ ] Payment date (e.g., by the 9th of each month)
- [ ] Working hours per week
- [ ] Overtime rules
- [ ] Pension contribution percentages (employer + employee)
- [ ] Keren hishtalmut percentages if applicable
- [ ] Vacation days per year
- [ ] Sick days accrual
- [ ] Dmei havra'a entitlement
- [ ] Hoda'at mukdemet (both directions)
- [ ] Severance (Section 14 or standard)
- [ ] Confidentiality scope and duration
- [ ] Non-compete scope, duration, and compensation
- [ ] IP and inventions assignment scope
- [ ] Governing law (must be Israeli) and jurisdiction (Israeli labor courts)
- [ ] Signature blocks for both parties
